...
import { useLanguage } from '@/context/LanguageContext'; // <-- STEP 3: Import Context
...
export default function MarketplaceNavbar() {
  ...
  // STEP 3: Get Translation Functions
  ...
  return (
    <header className="sticky top-0 w-full z-50 flex justify-between items-center px-4 md:px-12 py-4 max-w-[1280px] mx-auto bg-surface border-b border-outline-variant shadow-sm transition-all duration-300">
      ...
      <div className="flex items-center gap-4">
        ...
        {/* STEP 3: THE LANGUAGE TOGGLE */}
        <div className="bg-surface-container-low border border-outline-variant rounded-full p-1 flex items-center shadow-sm">
          <button 
            onClick={() => setLocale('en')}
            className={`px-3 py-1 text-xs font-bold rounded-full transition-all ${locale === 'en' ? 'bg-primary text-white shadow-sm' : 'text-on-surface-variant hover:text-on-surface'}`}
          >
            EN
          </button>
          <button 
            onClick={() => setLocale('hi')}
            className={`px-3 py-1 text-xs font-bold rounded-full transition-all ${locale === 'hi' ? 'bg-primary text-white shadow-sm' : 'text-on-surface-variant hover:text-on-surface'}`}
          >
            HI
          </button>
        </div>
