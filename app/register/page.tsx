...
export default function RegisterPage() {
  ...
  const [formData, setFormData] = useState({
    fullName: '', email: '', password: '', kissanId: '', aadharId: '', farmName: '', contactNumber: '', location: '', adminSecret: ''
  });
  const [aadhaarFile, setAadhaarFile] = useState<File | null>(null);

  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    if (role === 'admin' && formData.adminSecret !== 'BGI2026') return alert("SECURITY ERROR: Invalid Admin Access Code.");
    setLoading(true);

    try {
      const { data: authData, error: authError } = await supabase.auth.signUp({
        email: formData.email, password: formData.password,
        options: { data: { role: role, full_name: formData.fullName, contact_number: formData.contactNumber } }
      });
      ...
      if (role === 'farmer') {
        ...
        if (aadhaarFile) {
          ...
          const { error: uploadError } = await supabase.storage.from('kyc_documents').upload(fileName, aadhaarFile);
          if (!uploadError) kycUrl = supabase.storage.from('kyc_documents').getPublicUrl(fileName).data.publicUrl;
        }

        const { error: dbError } = await supabase.from('farm_profiles').insert({
          id: authData.user.id, full_name: formData.fullName, farm_name: formData.farmName, contact_number: formData.contactNumber, location: formData.location, kissan_id: formData.kissanId, apaar_id: formData.aadharId, kyc_document_url: kycUrl, verification_status: 'pending'
        });
  ...
  return (
    <div className="flex min-h-screen bg-white font-sans">
      ...
      {/* LEFT COLUMN: THE FORM (Scrolls naturally if tall) */}
            ...
            {role === 'farmer' && (
              <div className="border-t-2 border-outline-variant/50 pt-6 mt-8 space-y-5 animate-in fade-in slide-in-from-top-4 duration-500">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-xs font-bold text-on-surface-variant">Phone Number</label>
                    <div className="relative group">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant material-symbols-outlined text-[16px] group-focus-within:text-[#D97706] transition-colors">call</span>
                      <input required type="tel" value={formData.contactNumber} onChange={(e) => setFormData({...formData, contactNumber: e.target.value})} className="w-full py-3 pl-9 pr-3 rounded-xl bg-surface border-2 border-outline-variant outline-none focus:border-[#D97706] focus:bg-white transition-all shadow-sm text-sm" placeholder="+91..." />
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-[#fe932c]/5 border-2 border-[#fe932c]/20 rounded-2xl p-6 space-y-5 mt-4">
                  ...
                  <div className="space-y-1.5 pt-2">
                    ...
                    <input required type="file" accept="image/*" onChange={(e) => setAadhaarFile(e.target.files?.[0] || null)} className="w-full text-sm file:mr-4 file:py-2 file:px-6 file:rounded-full file:border-0 file:font-bold file:bg-[#904d00] file:text-white hover:file:brightness-110 hover:file:-translate-y-0.5 file:transition-all file:shadow-md cursor-pointer text-[#904d00]" />
                  </div>
                </div>
              </div>
            )}
      ...
      {/* RIGHT COLUMN: THE VISUAL BANNER (Fixed/Sticky) */}
      <div className="hidden lg:block lg:w-1/2 fixed right-0 top-0 bottom-0 bg-surface-container-high">
        {/* A beautiful new agricultural image for the registration side */}
        <img 
